...
int movimiento(int f,int c,tipoCasilla** matrix,char orden,int filas,int columnas, int mines);
...
int main(int argc,char* argv[]){
	...
	while(State==1){
		...
		printf("\nQuedan %d minas.\n",Mines);
		printf("'a' : abrir casilla | 'm' : marcar casilla | 'd' : desmarcar casilla | 'n' : nueva mina |\n");
		printf("Ingresa tu movimiento (ej. '2 2 a'): ");
		scanf("%d %d %c",&F,&C,&Orden);
		Mines=calcMines(Mines,Orden,F,C,Matrix);
		State=movimiento(F,C,Matrix,Orden,Filas,Columnas,Mines);
...
int movimiento(int f,int c,tipoCasilla** matrix,char orden,int filas,int columnas, int mines){
	...
	else if(orden=='n' && matrix[f][c].estado!=1){
		matrix[f][c].tipo='m';
		asignarNumeros(filas,columnas,mines,matrix);
		return 1;
	}
...
int calcMines(int mines,char orden,int f,int c,tipoCasilla** matrix){
	if(orden=='m'){
		...
		if(matrix[f][c].estado!=1 && matrix[f][c].tipo!='m'){
			mines++;
			return mines;
		}
	}
